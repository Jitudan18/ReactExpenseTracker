...
export const Adduser = () => {
  ...
  const stateOnChangeHandler = async (e) => {
    ...
    await axios
      .get(`http://localhost:4000/citiesbystateid/${stateid}`)
      .then((res) => {
        setcityList(res.data.data);
  ...
  const submit = (e) => {
    ...
    if (role == "62493c6379dd4902ea8995bc") {
      axios.post("http://localhost:4000/vendor", Data2).then((res) => {
      ...
      axios.post("http://localhost:4000/users", Data1).then((res) => {
        console.log(res.status);
        console.log(res.data);
